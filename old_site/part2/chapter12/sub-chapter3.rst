...
As we learned in Section 1.7, **availability** refers to the fraction of time your site is 
...
(Nygard 2007) that less-disciplined sites provide closer to “two eights” (88.0%).

Why might your app be unavailable? For one thing, it may have crashed because of an unexpected 
...
and write code to handle them. Here are three examples:
...
*rolling reboot*, and most PaaS platforms employ some variant of it.

Overprovisioning is often used in anticipation of crash recovery and rolling reboot. The idea 
...
Since responsiveness has such a large effect on user behavior, SaaS operators carefully monitor 
...
Of course, the total response time perceived by the users includes many factors beyond your 
...
on measuring and improving responsiveness in those components.
...
this service on top of Amazon EC2.

As we will see, a key insight that helps us is that *the same problems that push us out of the 
...
What are the thresholds for user satisfaction on responsiveness? A classic 1968 study from the 
...
**Self-Check 12.3.1.** *For a SaaS app to scale to large numbers of users, it must maintain 
its ____ and ____ as the number of users increases, without increasing the ____.*

    Availability; responsiveness; cost per user

**Self-Check 12.3.2.** *True or False: From the perspective of responsiveness, faster 
is always better.*

    False. Faster than 100 ms is not perceptible to people, and people abandon sites only when 
    responsiveness is slows to 8 seconds or worse.